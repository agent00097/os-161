...
static struct process *process_table[MAX_PROCESS_NUMBERS];

void process_table_initialization(void) {
    int i=0;
    for(i=1; i<MAX_PROCESS_NUMBERS; i++) {
...
thread_create(const char *name)
{
    ...
    DEBUG(DB_THREADS, "creating thread %s.\n",thread->t_name);
	...
	// Every thread starts at priority 1
	thread->priority = PRI_1;

    thread->pid = generate_pid(thread);
    return thread;
}

pid_t generate_pid(struct thread *here) {
	...
	snprintf(name, sizeof(name), "thread_cv%d", einstein);
	process_table[i]->sem_th = sem_create(name, 0);
	process_table[i]->parent_pid = -1;
	process_table[i]->exit_code = 0;
	process_table[i]->exit_status = 0;
	process_table[i]->the_thread = here;
	// snprintf(name, sizeof(name), "thread_lock%d", einstein);
	// process_table[i]->process_lock = lock_create(name);
	process_table[i]->the_thread->pid = einstein;
	return einstein;
}

struct process* get_child_process(pid_t pid) {
	int i = 0;
	for(i = 0; i < MAX_PROCESS_NUMBERS; i++) {
...
void deallocate_pid(pid_t pid) {
    
    sem_destroy(process_table[pid]->sem_th);
    // lock_destroy(process_table[pid]->process_lock);
        
    kfree(process_table[pid]->the_thread);    
...
thread_destroy(struct thread *thread)
{
	...
	// kfree(process_table[thread->pid]);
	// process_table[thread->pid] = NULL;

	if (thread->t_stack) {
		kfree(thread->t_stack);
	}
	DEBUG(DB_THREADS, "destroying thread %s.\n",thread->t_name);
	kfree(thread->t_name);
	kfree(thread);
...
exorcise(void)
{
    ...
    for (i=0; i<array_getnum(zombies); i++) {
        ...
        DEBUG(DB_THREADS, "exorcising thread %s.\n",z->t_name);
        assert(z!=curthread);
        thread_destroy(z);
    }
...
//
{
	...
	DEBUG(DB_THREADS, "new thread forked, having name %s.\n",newguy->t_name);   
	 ...
	 */
	numthreads++;
	DEBUG(DB_THREADS, "there are now %d threads running.\n",numthreads);   
...
mi_switch(threadstate_t nextstate)
{
    ...
    if (nextstate==S_READY) {
		...
		// If a thread yields, lower its priority
        curr_pri = cur->priority;
		if (curr_pri == 1) {
			curr_pri++;
        	cur->priority = curr_pri;
		} 
...
thread_wakeup_single(const void *addr)
{
	...
	// Grab the lowest numbered sleeper thread
	for (i=0; i<array_getnum(sleepers); i++) {
		struct thread *t = array_getguy(sleepers, i);
...
thread_wakeup(const void *addr)
{
    ...
    for (i=0; i<array_getnum(sleepers); i++) {
        ...
        if (t->t_sleepaddr == addr) {
			...
			// when we're waking up a thread, increase its priority
			int p = t->priority;
			p--;
			t->priority = p;
